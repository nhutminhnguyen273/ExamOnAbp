...
namespace ExamOnAbp.Shared.Hosting.Microservices.DbMigrations
{
    public abstract class PendingMigrationsCheckerBase : ITransientDependency
    {
        public async Task TryAsync(Func<Task> task, int retryCount = 3)
        {
            ...
            catch ( Exception ex )
            {
                ...
                Log.Warning($"{ex.GetType().Name} has been throw. The operation will be tried {retryCount} times more. Exception:\n{ex.Message}");
